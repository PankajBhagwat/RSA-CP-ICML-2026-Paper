...
def run_imagenet_experiment(data: dict, comp: dict, *, figure: str, alpha_values, n_cal_values, n_ref_values) -> pd.DataFrame:
    rows = []
        ...
        for n_cal in n_cal_values:
            for n_ref in n_ref_values:
                for run_id, run_seed in enumerate(seeds):
                    ...
                    state = prepare_rsacp_state(s_real, s_ref, alpha=alpha, beta=BETA_IMAGENET, use_ot=True)
